...
polynomial_degrees = [2, 3]; % Quadratic and cubic
n_int = 10; % Number of integration points

for p_idx = 1:length(polynomial_degrees)
    pp = polynomial_degrees(p_idx); % Current polynomial degree
    n_en = pp + 1; % Number of element or local nodes

    % Store errors and mesh sizes
    num_elements = 2:2:16; % Number of elements
    ...
    figure;
    loglog(h_values, L2_errors, '-o', 'LineWidth', 2, 'DisplayName', sprintf('L2 Error (pp=%d)', pp));
    hold on;
    loglog(h_values, H1_errors, '-x', 'LineWidth', 2, 'DisplayName', sprintf('H1 Error (pp=%d)', pp));
    hold off;
    grid on;
    xlabel('Mesh size h');
    ylabel('Error');
    legend show;
    title(sprintf('Error vs Mesh Size (pp=%d)', pp));
end
